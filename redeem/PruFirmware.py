...
class PruFirmware:
  def __init__(self, firmware_source_file0, binary_filename0, firmware_source_file1,
    ...
    if os.path.splitext(self.binary_filename1)[1] != '.bin':
      logging.error('Invalid binary output filename on file 1. '
                    'It should have the .bin extension.')
      raise RuntimeError('Invalid binary output filenameon file 1. '
                         'It should have the .bin extension.')
  ...
  def is_needing_firmware_compilation(self):
    ...
    if os.path.exists(self.binary_filename0):
      ...
      fw_src_mtime = os.path.getmtime(self.firmware_source_file0)    # modif time of firmware source
    ...
    if os.path.exists(self.binary_filename1):
      ...
      fw_src_mtime = os.path.getmtime(self.firmware_source_file1)    # modif time of firmware source
      if fw_mtime >= config_mtime and fw_mtime >= fw_src_mtime:
        # already up to date
        ret1 = False
  ...
  def build_firmware(self, input, output):
    ...
    compile_cmd = [
        self.compiler, '-v3', '-O3', '--hardware_mac=on', '--endian=little',
        '-I/usr/src/pru-software-support-package/include/am335x', '-I/usr/share/ti/cgt-pru/include',
        '--obj_directory', '/tmp', input
    ]
    ...
    link_cmd = [
        self.compiler, '-v3', '-O3', '--hardware_mac=on', '--endian=little', '-z', '-i',
        '/usr/share/ti/cgt-pru/lib', '--stack_size=0x100', '--heap_size=0x100', '--rom_model', '-o',
        elf_output, obj_output, self.linker_cmdfile
    ]
  ...
  def make_config_file(self):
    ...
    with open(configFile_0, 'w') as configFile:
      ...
      revision = self.printer.characteristics.replicape_revision.strip('0')
