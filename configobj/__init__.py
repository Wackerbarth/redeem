  ...
  def walk(self, function, raise_errors=True, call_on_sections=False, **keywargs):
    ...
    for i in range(len(self.sections)):
      ...
      out[entry] = self[entry].walk(
          function, raise_errors=raise_errors, call_on_sections=call_on_sections, **keywargs)
    return out
...
class ConfigObj(Section):
    ...
    if self._errors:
      if len(self._errors) > 1:
        ...
        if len(self._errors) > self.MAX_PARSE_ERROR_DETAILS:
          msg.append("{} more error(s)!".format(len(self._errors) - self.MAX_PARSE_ERROR_DETAILS))
  ...
  def __repr__(self):
    ...
    return ('{}({{{}}})'.format(
        self.__class__.__name__, ', '.join([('{}: {}'.format(repr(key), repr(_getval(key))))
                                            for key in (self.scalars + self.sections)])))
  ...
  def _parse(self, infile):
    ...
    while cur_index < maxline:
      ...
      if mat is None:
        self._handle_error(
            'Invalid line ({!r}) (matched as neither section nor keyword)'.format(line), ParseError,
            infile, cur_index)
      else:
        # is a keyword value
  ...
  def _quote(self, value, multiline=True):
    ...
    hash_triple_quote = multiline and not need_triple and ("'" in value) and ('"' in value) and (
        '#' in value)
    ...
    if check_for_single:
      ...
      # for normal values either single or double quotes will do
        ...
        raise ConfigObjError('Value cannot be safely quoted: {!r}'.format(value))
      elif ((value[0] not in wspace_plus) and (value[-1] not in wspace_plus)
            and (',' not in value)):
        quot = noquot
      else:
  ...
  def _write_line(self, indent_string, entry, this_entry, comment):
    ...
    if not self.unrepr:
      ...
      val = repr(this_entry)
    return '%s%s%s%s%s' % (indent_string, self._decode_element(self._quote(entry, multiline=False)),
  ...
  def _write_marker(self, indent_string, depth, entry, comment):
    ...
    if entry_str and title[0] in '\'"' and title[1:-1] == entry_str:
      ...
      title = entry_str
    return '%s%s%s%s%s' % (indent_string, self._a_to_u('[' * depth), title, self._a_to_u(
        ']' * depth), self._decode_element(comment))
  ...
  def write(self, outfile=None, section=None):
    ...
    if (self.filename is None) and (outfile is None):
      ...
      if (self.BOM
          and ((self.encoding is None) or (BOM_LIST.get(self.encoding.lower()) == 'utf_8'))):
        # Add the UTF8 BOM
        if not out:
          out.append('')
